...
function getCartElements() {
  ...
  cartCloseButton = document.querySelector(
    "#cart-close-button",
  );
  ...
  cartProductsContainer = document.querySelector(
    "#cart-products",
  );


  cartProductTemplate = document.querySelector(
  ...
  cartTotalItems = document.querySelector(
    "#cart-total-items",
  );


  cartSummaryItems = document.querySelector(
    "#cart-summary-items",
  );


  cartTotalPrice = document.querySelector(
    "#cart-total-price",
  );


  cartWhatsappButton = document.querySelector(
...
async function loadCartHTML() {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `No se pudo cargar cart.html: ${response.status}`,
      );
    }
...
function createCartProductElement(item) {
  ...
  const fragment =
    cartProductTemplate.content.cloneNode(true);


  const cartProduct = fragment.querySelector(
    ".cart-product",
  );
  const sku = fragment.querySelector(
  ...
  const brand = fragment.querySelector(
  ...
  const variant = fragment.querySelector(
    "[data-cart-product-variant]",
  );


  const quantity = fragment.querySelector(
    "[data-cart-product-quantity]",
  );


  const unitPrice = fragment.querySelector(
    "[data-cart-product-unit-price]",
  );


  const totalPrice = fragment.querySelector(
    "[data-cart-product-total-price]",
  );


  const productLinks = fragment.querySelectorAll(
    "[data-cart-product-link]",
  );


  const productUrl =
    `/src/pages/producto.html?id=${encodeURIComponent(
      item.id,
    )}`;
...
function renderCartProducts() {
  ...
  cartItems.forEach((item) => {
    const productElement =
      createCartProductElement(item);


    if (productElement) {
      cartProductsContainer.appendChild(
        productElement,
      );
    }
  });
...
function updateWhatsappButton() {
  ...
  if (cartIsEmpty) {
    ...
    cartWhatsappButton.setAttribute(
      "aria-disabled",
      "true",
    );
...
function setupCartProductEvents() {
  cartProductsContainer?.addEventListener(
    "click",
    (event) => {
      ...
      const productId =
        productElement.dataset.cartProductId;


      const increaseButton = event.target.closest(
        "[data-cart-product-increase]",
      );


      const decreaseButton = event.target.closest(
        "[data-cart-product-decrease]",
      );


      const deleteButton = event.target.closest(
        "[data-cart-product-delete]",
      );
...
function setupProductCardButtons() {
  document.addEventListener("click", (event) => {
    ...
    const productCard = addButton.closest(
    ...
    if (!productId) {
      console.error(
        "La tarjeta no contiene data-product-id.",
      );
...
export async function initCart() {
  ...
  if (!cartPanel || !cartProductTemplate) {
    console.error(
      "No se encontraron los elementos necesarios del carrito.",
    );
